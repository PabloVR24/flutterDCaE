...
enum PasswordRepeatError { empty, length, format, equals }
...
class PasswordRepeat extends FormzInput<String, PasswordRepeatError> {
  ...
  String? get errorMessage {
    ...
    if (displayError == PasswordRepeatError.empty) {
      return 'El campo es requerido';
    }
    if (displayError == PasswordRepeatError.length) {
      return 'Mínimo 6 caracteres';
    }
    if (displayError == PasswordRepeatError.equals) {
      return 'Las contraseñas no coinciden';
    }
    if (displayError == PasswordRepeatError.format) {
  ...
  PasswordRepeatError? validator(String value) {
    ...
    if (value != Password.dirty(value)) return PasswordRepeatError.equals;
